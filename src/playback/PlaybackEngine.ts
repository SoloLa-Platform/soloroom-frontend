...
export default class PlaybackEngine {
  ...
  public getPlaybackInstrument(voiceId: number): PlaybackInstrument {
    ...
    return this.availableInstruments.find(
  ...
  public async loadScore(osmd: OpenSheetMusicDisplay): Promise<void> {
    this.ready = false;
  ...
  private fallbackToPiano(i: Instrument) {
    // console.warn(
    //   `Can't find playback instrument for midiInstrumentId ${
    //     i.MidiInstrumentId

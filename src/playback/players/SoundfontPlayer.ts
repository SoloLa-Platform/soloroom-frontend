...
export class SoundfontPlayer implements InstrumentPlayer {
  ...
  public play: (midiId: string | number, options: NotePlaybackStyle) => void;
  ...
  constructor() {
    this.instruments = midiInstruments
      .filter((i) =>
        supportedSoundfontInstruments.includes(
          this.getSoundfontInstrumentName(i[1]),
        ),
      )
      .map((i) => ({
        midiId: i[0],
  ...
  public async load(midiId: number) {
    const instrument = this.instruments.find((i) => i.midiId === midiId);
    if (!instrument) {
      throw new Error(
        'SoundfontPlayer does not support midi instrument ID ' + midiId,
      );
    }
    ...
    const player = await Soundfont.instrument(
      // @ts-ignore
      this.audioContext,
      this.getSoundfontInstrumentName(
        instrument.name,
      ) as Soundfont.InstrumentName,
    );
    this.players.set(midiId, player);
  }

  public stop(midiId: number) {
    if (!this.players.has(midiId)) {
      return;
    }
  ...
  private verifyPlayerLoaded(midiId: number) {
    if (!this.players.has(midiId)) {
      throw new Error(
        'No soundfont player loaded for midi instrument ' + midiId,
      );
    }
  }
